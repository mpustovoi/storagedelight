...
{
    private static final Component DEFAULT_NAME = Component.translatable("container.storagedelight.small_drawers");
    private NonNullList<ItemStack> items;
    private final ContainerOpenersCounter openersCounter;

    public SmallDrawersBlockEntity(BlockPos pos, BlockState blockState) {
        super(EntityTypesRegistry.SMALL_DRAWERS.get(), pos, blockState);
    ...
    private void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), (BlockState)state.setValue(SmallDrawersBlock.OPEN, open), 3);
    }

    private void playSound(BlockState state, SoundEvent event) {
        Vec3i direction = ((Direction)state.getValue(SmallDrawersBlock.FACING)).getUnitVec3i();
