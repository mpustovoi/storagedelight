...
public class DrawerDoorBlockEntity extends LootableContainerBlockEntity {
    ...
    public void tick() {
        ...
        if (this.viewerManager.getViewerCount() > 0) {
            ...
            if (!(blockstate.getBlock() instanceof DrawerDoorBlock)) {
                markRemoved();
                return;
            }

            boolean flag = blockstate.get(DrawerDoorBlock.OPEN);
            if (flag) {
    ...
    private void setOpen(BlockState state, boolean open) {
        Objects.requireNonNull(getWorld()).setBlockState(getPos(), state.with(DrawerDoorBlock.OPEN, open));
    }

    private void playSound(BlockState state, SoundEvent sound) {
        Vec3i vec3i = state.get(DrawerDoorBlock.FACING).getVector();
        BlockPos pos = getPos();
