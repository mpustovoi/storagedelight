...
public class DrawerDoorBlockEntity extends RandomizableContainerBlockEntity
{
    ...
    void updateBlockState(BlockState state, boolean open) {
        if (level != null) {
            this.level.setBlock(this.getBlockPos(), state.setValue(DrawerDoorBlock.OPEN, open), 3);
        }
    }
    ...
    private void playSound(BlockState state, SoundEvent sound) {
        ...
        Vec3i drawerFacingVector = state.getValue(DrawerDoorBlock.FACING).getNormal();
