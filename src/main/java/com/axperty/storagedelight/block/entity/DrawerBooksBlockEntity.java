...
public class DrawerBooksBlockEntity extends RandomizableContainerBlockEntity
{
    ...
    private ContainerOpenersCounter openersCounter = new ContainerOpenersCounter()
    {
        protected void onOpen(Level level, BlockPos pos, BlockState state) {
            DrawerBooksBlockEntity.this.playSound(state, SoundEvents.BARREL_OPEN);
            DrawerBooksBlockEntity.this.updateBlockState(state, true);
        }

        protected void onClose(Level level, BlockPos pos, BlockState state) {
            DrawerBooksBlockEntity.this.playSound(state, SoundEvents.BARREL_CLOSE);
            DrawerBooksBlockEntity.this.updateBlockState(state, false);
        }
    ...
    public DrawerBooksBlockEntity(BlockPos pos, BlockState state) {
        super(EntityTypesRegistry.DRAWER_BOOKS.get(), pos, state);
    }
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.storagedelight.drawer_books");
    }
    ...
    void updateBlockState(BlockState state, boolean open) {
        if (level != null) {
            this.level.setBlock(this.getBlockPos(), state.setValue(DrawerBooksBlock.OPEN, open), 3);
        }
    }
    ...
    private void playSound(BlockState state, SoundEvent sound) {
        ...
        Vec3i drawerFacingVector = state.getValue(DrawerBooksBlock.FACING).getNormal();
        double x = (double) worldPosition.getX() + 0.5D + (double) drawerFacingVector.getX() / 2.0D;
        double y = (double) worldPosition.getY() + 0.5D + (double) drawerFacingVector.getY() / 2.0D;
        double z = (double) worldPosition.getZ() + 0.5D + (double) drawerFacingVector.getZ() / 2.0D;
