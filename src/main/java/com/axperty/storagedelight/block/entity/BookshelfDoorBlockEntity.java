...
public class BookshelfDoorBlockEntity extends LootableContainerBlockEntity {
    ...
    public BookshelfDoorBlockEntity(BlockPos blockPos, BlockState blockState) {
        this(BlockEntityTypesRegistry.BOOKSHELF_DOOR.get(), blockPos, blockState);
    }
    ...
    public void tick() {
        ...
        if (this.viewerManager.getViewerCount() > 0) {
            ...
            if (!(blockstate.getBlock() instanceof BookshelfDoorBlock)) {
                markRemoved();
                return;
            }

            boolean flag = blockstate.get(BookshelfDoorBlock.OPEN);
            if (flag) {
    ...
    private void setOpen(BlockState state, boolean open) {
        Objects.requireNonNull(getWorld()).setBlockState(getPos(), state.with(BookshelfDoorBlock.OPEN, open));
    }
