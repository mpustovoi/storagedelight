...
public class BookshelfDoorBlockEntity extends RandomizableContainerBlockEntity
{
    ...
    private ContainerOpenersCounter openersCounter = new ContainerOpenersCounter()
    {
        protected void onOpen(Level level, BlockPos pos, BlockState state) {
            ...
            BookshelfDoorBlockEntity.this.updateBlockState(state, true);
        }

        protected void onClose(Level level, BlockPos pos, BlockState state) {
            BookshelfDoorBlockEntity.this.playSound(state, SoundEvents.WOODEN_TRAPDOOR_CLOSE);
            BookshelfDoorBlockEntity.this.updateBlockState(state, false);
        }
    ...
    void updateBlockState(BlockState state, boolean open) {
        if (level != null) {
            this.level.setBlock(this.getBlockPos(), state.setValue(BookshelfDoorBlock.OPEN, open), 3);
        }
    }
    ...
    private void playSound(BlockState state, SoundEvent sound) {
        ...
        Vec3i bookshelfFacingVector = state.getValue(BookshelfDoorBlock.FACING).getNormal();
        double x = (double) worldPosition.getX() + 0.5D + (double) bookshelfFacingVector.getX() / 2.0D;
        double y = (double) worldPosition.getY() + 0.5D + (double) bookshelfFacingVector.getY() / 2.0D;
        double z = (double) worldPosition.getZ() + 0.5D + (double) bookshelfFacingVector.getZ() / 2.0D;
