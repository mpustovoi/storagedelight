...
public class CabinetCountertopBlockEntity extends LootableContainerBlockEntity {
    ...
    public CabinetCountertopBlockEntity(BlockPos blockPos, BlockState blockState) {
        this(EntityTypesRegistry.COUNTERTOP, blockPos, blockState);
    }

    private CabinetCountertopBlockEntity(BlockEntityType<?> type, BlockPos blockPos, BlockState blockState) {
        ...
        this.viewerManager = new ViewerCountManager() {
            protected void onContainerOpen(World world, BlockPos pos, BlockState state) {
                CabinetCountertopBlockEntity.this.playSound(state, SoundEvents.BLOCK_BARREL_OPEN);
                CabinetCountertopBlockEntity.this.setOpen(state, true);
            }

            protected void onContainerClose(World world, BlockPos pos, BlockState state) {
                CabinetCountertopBlockEntity.this.playSound(state, SoundEvents.BLOCK_BARREL_CLOSE);
                CabinetCountertopBlockEntity.this.setOpen(state, false);
            }
            ...
            protected boolean isPlayerViewing(PlayerEntity player) {
                if (player.currentScreenHandler instanceof GenericContainerScreenHandler genericContainerScreenHandler) {
                    ...
                    return inventory == CabinetCountertopBlockEntity.this;
                } else {
                    return false;
                }
            }
    ...
    protected Text getContainerName() {
        return Text.translatable("container.storagedelight.cabinet_countertop");
    }
    ...
    public void tick() {
        ...
        if (this.viewerManager.getViewerCount() > 0) {
            ...
            if (!(blockstate.getBlock() instanceof CabinetCountertopBlock)) {
                markRemoved();
                return;
            }

            boolean flag = blockstate.get(CabinetCountertopBlock.OPEN);
            if (flag) {
    ...
    private void setOpen(BlockState state, boolean open) {
        Objects.requireNonNull(getWorld()).setBlockState(getPos(), state.with(CabinetCountertopBlock.OPEN, open));
    }

    private void playSound(BlockState state, SoundEvent sound) {
        Vec3i vec3i = state.get(CabinetCountertopBlock.FACING).getVector();
        BlockPos pos = getPos();
