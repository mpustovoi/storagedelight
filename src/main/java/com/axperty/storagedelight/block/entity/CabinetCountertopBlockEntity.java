...
public class CabinetCountertopBlockEntity extends RandomizableContainerBlockEntity
{
    ...
    private ContainerOpenersCounter openersCounter = new ContainerOpenersCounter()
    {
        protected void onOpen(Level level, BlockPos pos, BlockState state) {
            CabinetCountertopBlockEntity.this.playSound(state, SoundEvents.BARREL_OPEN);
            CabinetCountertopBlockEntity.this.updateBlockState(state, true);
        }

        protected void onClose(Level level, BlockPos pos, BlockState state) {
            CabinetCountertopBlockEntity.this.playSound(state, SoundEvents.BARREL_CLOSE);
            CabinetCountertopBlockEntity.this.updateBlockState(state, false);
        }
    ...
    public CabinetCountertopBlockEntity(BlockPos pos, BlockState state) {
        super(EntityTypesRegistry.COUNTERTOP.get(), pos, state);
    }
    ...
    void updateBlockState(BlockState state, boolean open) {
        if (level != null) {
            this.level.setBlock(this.getBlockPos(), state.setValue(CabinetCountertopBlock.OPEN, open), 3);
        }
    }
    ...
    private void playSound(BlockState state, SoundEvent sound) {
        ...
        Vec3i cabinetCountertopFacingVector = state.getValue(CabinetCountertopBlock.FACING).getNormal();
        double x = (double) worldPosition.getX() + 0.5D + (double) cabinetCountertopFacingVector.getX() / 2.0D;
        double y = (double) worldPosition.getY() + 0.5D + (double) cabinetCountertopFacingVector.getY() / 2.0D;
        double z = (double) worldPosition.getZ() + 0.5D + (double) cabinetCountertopFacingVector.getZ() / 2.0D;
