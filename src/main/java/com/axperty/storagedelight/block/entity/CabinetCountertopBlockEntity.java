...
public class CabinetCountertopBlockEntity extends LootableContainerBlockEntity {
    private static final Text CONTAINER_NAME_TEXT = Text.translatable("container.storagedelight.cabinet_countertop");
    private DefaultedList<ItemStack> inventory;
    private final ViewerCountManager stateManager;

    public CabinetCountertopBlockEntity(BlockPos pos, BlockState state) {
        super(EntityTypesRegistry.COUNTERTOP, pos, state);
    ...
    void setOpen(BlockState state, boolean open) {
        this.world.setBlockState(this.getPos(), (BlockState)state.with(CabinetCountertopBlock.OPEN, open), 3);
    }

    void playSound(BlockState state, SoundEvent soundEvent) {
        Vec3i vec3i = ((Direction)state.get(CabinetCountertopBlock.FACING)).getVector();
