...
{
    private static final Component DEFAULT_NAME = Component.translatable("container.storagedelight.cabinet_variant");
    private NonNullList<ItemStack> items;
    private final ContainerOpenersCounter openersCounter;

    public CabinetVariantBlockEntity(BlockPos pos, BlockState blockState) {
        super(EntityTypesRegistry.CABINET_VARIANT.get(), pos, blockState);
    ...
    private void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), (BlockState)state.setValue(CabinetVariantBlock.OPEN, open), 3);
    }

    private void playSound(BlockState state, SoundEvent event) {
        Vec3i direction = ((Direction)state.getValue(CabinetVariantBlock.FACING)).getUnitVec3i();
